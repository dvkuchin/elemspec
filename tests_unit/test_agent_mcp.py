...
class MCPСерверTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_публикует_узкие_инструменты(self) -> None:
        ...
        self.assertIn("заголовок формы", варианты)
        self.assertIn("заголовок диалога", варианты)
    ...
    async def test_select_html_option_требует_ref_и_value(self) -> None:
        async with Client(self.сервер, raise_exceptions=True) as клиент:
            ответ = await клиент.call_tool(
                "browser_action",
                {
                    "browser_session": "missing",
                    "operation": "select-html-option",
                },
            )
        self.assertTrue(ответ.is_error)
        self.assertIn("select-html-option требует ref", ответ.content[0].text)
