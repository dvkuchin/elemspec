...
class НегативнаяМутацияTest(unittest.TestCase):
    ...
    def test_портит_ожидаемое_значение_поля(self) -> None:
        ...
        мутация = выбрать_мутацию(feature)
        self.assertEqual("проверить_значение_поля", мутация["действие"])
        self.assertIn(
    ...
    def test_портит_позитивную_проверку_строки_таблицы(self) -> None:
        feature = (
            '# language: ru\nФункция: Таблица\n  Сценарий: Строка\n'
        ...
        мутация = выбрать_мутацию(feature)
        self.assertEqual("проверить_строки_таблицы", мутация["действие"])
        self.assertIn('"Иван__ELEMPWT_NEGATIVE_', мутация["feature"])

    def test_отсутствие_строки_инвертируется_в_наличие(self) -> None:
        feature = (
            '# language: ru\nФункция: Таблица\n  Сценарий: Строка\n'
            '    Тогда в таблице "ОсновнаяТаблица" нет строки, где '
            'колонка "Наименование" имеет значение "Иван"\n'
        )
        мутация = выбрать_мутацию(feature)
        self.assertIn(
            'в таблице "ОсновнаяТаблица" есть строка, где',
            мутация["feature"],
        )
