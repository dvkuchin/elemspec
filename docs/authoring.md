...
ElemSpec поддерживает два режима ИИ-автора. Обычная задача Codex удобна для ежедневной
работы; отдельный строгий Codex CLI технически отключает все инструменты кроме
`elemspec mcp`.
