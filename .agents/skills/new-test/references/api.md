...
изменения DOM запросить новый `snapshot`. Операция `click` для пункта навигации
сама наводит указатель и ждёт два кадра готовности DOM; `sleep` и ручной повтор
клика не нужны.
